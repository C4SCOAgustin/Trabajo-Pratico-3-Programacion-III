...
public class EquipoTest {
	...
	@Test
	public void testQuitarPersonaInexistenteNoCambiaEquipo() {
		equipo.agregar(juan);
	...
	@Test
	public void testQuitarUsaIgualdadPorNombre() {
		equipo.agregar(juan);
		Persona mismoNombre = new Persona("Juan", Rol.TESTER, 1);

		equipo.quitar(mismoNombre);

		assertTrue(equipo.estaVacio());
	}

	@Test
	public void testConstructorCopiaListaOriginal() {
		List<Persona> personas = new ArrayList<>();
		personas.add(juan);
		Equipo eq = new Equipo(personas);
		...
		assertEquals(1, eq.getIntegrantes().size());
		assertFalse(eq.getIntegrantes().contains(maria));
	}

	@Test
	public void testCantidadEnRolEquipoVacio() {
		for (Rol rol : Rol.values()) {
			assertEquals(0, equipo.cantidadEnRol(rol));
		}
	}

	@Test
	public void testPuntajeTotalConCalificacionesMinimaYMaxima() {
		equipo.agregar(new Persona("Min", Rol.TESTER, Persona.CALIFICACION_MINIMA));
		equipo.agregar(new Persona("Max", Rol.PROGRAMADOR, Persona.CALIFICACION_MAXIMA));

		assertEquals(Persona.CALIFICACION_MINIMA + Persona.CALIFICACION_MAXIMA,
				equipo.puntajeTotal());
	}

	@Test
	public void testToStringEquipoVacioIncluyePuntajeCero() {
		String str = equipo.toString();

		assertTrue(str.contains("Puntaje total: 0"));
	}
}
