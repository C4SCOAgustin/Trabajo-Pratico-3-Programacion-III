...
public class IncompatibilidadTest {
	...
	@Test
	public void testGettersMantienenOrdenOriginal() {
		assertSame(juan, incomp.getA());
		assertSame(maria, incomp.getB());
	}

	@Test(expected = IllegalArgumentException.class)
	public void testPersonasIgualesPorNombre() {
		Persona juanOtroRol = new Persona("Juan", Rol.ARQUITECTO, 1);

		new Incompatibilidad(juan, juanOtroRol);
	}

	@Test
	public void testInvolucraPersonaNullRetornaFalse() {
		assertFalse(incomp.involucra((Persona) null));
	}

	@Test
	public void testInvolucraParConNullRetornaFalse() {
		assertFalse(incomp.involucra(juan, null));
		assertFalse(incomp.involucra(null, maria));
	}

	@Test
	public void testToStringUsaSoloNombres() {
		String str = incomp.toString();

		assertFalse(str.contains("Programador"));
		assertFalse(str.contains("Arquitecto"));
		assertEquals(juan.getNombre() + " <-> " + maria.getNombre(), str);
	}
}
