...
public class EstadoAplicacionTest {
	...
	@Test
	public void testConstructorCopiaParesProfundamente() {
		String[] parOriginal = paresIncompatibles.get(0);
		EstadoAplicacion estado = new EstadoAplicacion(personas, paresIncompatibles, requerimientos);

		parOriginal[0] = "Modificado";

		assertEquals("Juan", estado.getParesIncompatibles().get(0)[0]);
	}

	@Test
	public void testGetParesIncompatiblesRetornaArreglosIndependientes() {
		EstadoAplicacion estado = new EstadoAplicacion(personas, paresIncompatibles, requerimientos);
		List<String[]> paresRecuperados = estado.getParesIncompatibles();

		paresRecuperados.get(0)[0] = "Modificado";

		assertEquals("Juan", estado.getParesIncompatibles().get(0)[0]);
	}

	@Test
	public void testOrdenDePersonasSeConserva() {
		personas.add(new Persona("Carlos", Rol.TESTER, 3));
		EstadoAplicacion estado = new EstadoAplicacion(personas, paresIncompatibles, requerimientos);

		assertEquals("Juan", estado.getPersonas().get(0).getNombre());
		assertEquals(personas.get(1).getNombre(), estado.getPersonas().get(1).getNombre());
		assertEquals("Carlos", estado.getPersonas().get(2).getNombre());
	}

	@Test
	public void testOrdenDeParesSeConserva() {
		paresIncompatibles.add(new String[] { "Carlos", "Ana" });
		EstadoAplicacion estado = new EstadoAplicacion(personas, paresIncompatibles, requerimientos);

		assertArrayEquals(paresIncompatibles.get(0), estado.getParesIncompatibles().get(0));
		assertArrayEquals(new String[] { "Carlos", "Ana" }, estado.getParesIncompatibles().get(1));
	}
}
