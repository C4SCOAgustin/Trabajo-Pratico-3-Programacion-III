...
public class RequerimientosTest {
	...
	@Test
	public void testSetCantidadSobrescribeValorAnterior() {
		req.setCantidad(Rol.PROGRAMADOR, 2);
		req.setCantidad(Rol.PROGRAMADOR, 5);

		assertEquals(5, req.getCantidad(Rol.PROGRAMADOR));
		assertEquals(5, req.total());
	}

	@Test
	public void testTotalConTodosLosRoles() {
		req.setCantidad(Rol.PROGRAMADOR, 4);
		req.setCantidad(Rol.ARQUITECTO, 3);
		req.setCantidad(Rol.TESTER, 2);
		req.setCantidad(Rol.LIDER_DE_PROYECTO, 1);

		assertEquals(10, req.total());
	}

	@Test
	public void testCopiarDesdeSobrescribeValoresPrevios() {
		Requerimientos origen = new Requerimientos();
		origen.setCantidad(Rol.TESTER, 2);
		req.setCantidad(Rol.PROGRAMADOR, 9);

		req.copiarDesde(origen);
	...
	@Test
	public void testCopiarDesdeMismoObjetoNoCambiaValores() {
		req.setCantidad(Rol.ARQUITECTO, 3);

		req.copiarDesde(req);

		assertEquals(3, req.getCantidad(Rol.ARQUITECTO));
	}

	@Test
	public void testToStringIncluyeTodosLosRoles() {
		req.setCantidad(Rol.PROGRAMADOR, 2);
		String str = req.toString();

		for (Rol rol : Rol.values()) {
			assertTrue(str.contains(rol.getNombre()));
		}
		assertTrue(str.contains("2"));
	}

	@Test
	public void testCantidadesAltas() {
		req.setCantidad(Rol.TESTER, 1000);

		assertEquals(1000, req.getCantidad(Rol.TESTER));
		assertEquals(1000, req.total());
	}
}
