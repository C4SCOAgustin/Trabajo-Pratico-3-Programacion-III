...
/**
 * Vista (capa Vista del MVC). Solo interfaz, sin lógica de negocio.
 * Implementa ModeloObserver para recibir notificaciones automáticas del modelo.
 */
public class VentanaPrincipal extends JFrame implements ModeloObserver {
    ...
    private static final int VENTANA_X = 100;
    private static final int VENTANA_Y = 100;
    private static final int VENTANA_ANCHO = 650;
    private static final int VENTANA_ALTO = 520;
    private static final int CALIFICACION_INICIAL = 3;
    private static final int REQUERIMIENTO_MINIMO = 0;
    private static final int REQUERIMIENTO_MAXIMO_UI = 50;
    private static final int REQUERIMIENTO_LIDER_INICIAL = 1;
    private static final int REQUERIMIENTO_ARQUITECTO_INICIAL = 1;
    private static final int REQUERIMIENTO_PROGRAMADOR_INICIAL = 2;
    private static final int REQUERIMIENTO_TESTER_INICIAL = 1;
    private static final int INCREMENTO_SPINNER = 1;
    ...
    public VentanaPrincipal(ModeloEquipo modelo, Controlador controlador) {
        ...
        setBounds(VENTANA_X, VENTANA_Y, VENTANA_ANCHO, VENTANA_ALTO);
    ...
    private JPanel crearPanelPersonas() {
        ...
        filaCalificacion.add(etiqueta("Calificación (" + Persona.CALIFICACION_MINIMA + "-"
                + Persona.CALIFICACION_MAXIMA + "):"), BorderLayout.WEST);
        spnCalificacion = new JSpinner(new SpinnerNumberModel(CALIFICACION_INICIAL,
                Persona.CALIFICACION_MINIMA, Persona.CALIFICACION_MAXIMA, INCREMENTO_SPINNER));
    ...
    private JPanel crearPanelRequerimientos() {
        ...
        spnLider = new JSpinner(new SpinnerNumberModel(REQUERIMIENTO_LIDER_INICIAL,
                REQUERIMIENTO_MINIMO, REQUERIMIENTO_MAXIMO_UI, INCREMENTO_SPINNER));
        panel.add(spnLider);
        configurarSpinnerSoloFlechas(spnLider);

        panel.add(etiqueta("Arquitectos:"));
        spnArquitecto = new JSpinner(new SpinnerNumberModel(REQUERIMIENTO_ARQUITECTO_INICIAL,
                REQUERIMIENTO_MINIMO, REQUERIMIENTO_MAXIMO_UI, INCREMENTO_SPINNER));
        panel.add(spnArquitecto);
        configurarSpinnerSoloFlechas(spnArquitecto);

        panel.add(etiqueta("Programadores:"));
        spnProgramador = new JSpinner(new SpinnerNumberModel(REQUERIMIENTO_PROGRAMADOR_INICIAL,
                REQUERIMIENTO_MINIMO, REQUERIMIENTO_MAXIMO_UI, INCREMENTO_SPINNER));
        panel.add(spnProgramador);
        configurarSpinnerSoloFlechas(spnProgramador);

        panel.add(etiqueta("Testers:"));
        spnTester = new JSpinner(new SpinnerNumberModel(REQUERIMIENTO_TESTER_INICIAL,
                REQUERIMIENTO_MINIMO, REQUERIMIENTO_MAXIMO_UI, INCREMENTO_SPINNER));
        panel.add(spnTester);
    ...
    private JPanel crearPanelResultado() {
        ...
        barraProgreso = new JProgressBar(PROGRESO_MINIMO, PROGRESO_COMPLETO);
    ...
    private void iniciarBarraProgreso() {
        ...
        progresoVisual = PROGRESO_MINIMO;
        inicioResolucion = System.currentTimeMillis();

        barraProgreso.setVisible(true);
        lblProgreso.setVisible(true);
        actualizarBarraProgresoMonotono(PROGRESO_MINIMO);
    ...
    private void avanzarBarraProgreso() {
        ...
        if (!faseFinalizacion) {
            ...
            int rango = PROGRESO_COMPLETO - progresoInicioFaseFinal;
            objetivo = progresoInicioFaseFinal
                    + (int) (rango * elapsedFinal / (double) DURACION_COMPLETAR_MS);
            objetivo = Math.min(PROGRESO_COMPLETO, objetivo);
        }

        actualizarBarraProgresoMonotono(objetivo);

        if (faseFinalizacion && progresoVisual >= PROGRESO_COMPLETO) {
            detenerTimerProgreso();
            completarResolucion();
        }
    }
    ...
    private void actualizarBarraProgresoMonotono(int porcentaje) {
        int valor = Math.max(PROGRESO_MINIMO, Math.min(PROGRESO_COMPLETO, porcentaje));
    ...
    private void completarResolucion() {
        actualizarBarraProgresoMonotono(PROGRESO_COMPLETO);
    ...
    // ---- Actualización de la UI (llamadas por el Observer) ----

    private void refrescarPersonas(List<Persona> personas) {
    ...
    private void refrescarIncompatibilidades(List<Incompatibilidad> incompatibilidades) {
